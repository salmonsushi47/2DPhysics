...
int main(void)
{
    unsigned short int x_res = 1366, y_res = 768;
    mainWindow.create(sf::VideoMode({x_res, y_res}), "Elastic Collisions", sf::Style::Titlebar); // (1366, 768) (1920, 1080)
    ...
    bool quadTree_visible = false;
    bool isMBDown[3]{};
    enum Mouse_Button:unsigned short int{LMB, RMB, MMB};
    while (mainWindow.isOpen())
    {
        Timer.restart();
        //note: All pointers break here
        while (const std::optional event = mainWindow.pollEvent())
        {
            ...
            {
                isMBDown[static_cast<unsigned short int>(btn->button)] = true;
                switch (static_cast<unsigned short int>(btn->button))
                {
                case LMB:
            ...
            if (const auto *key = event->getIf<sf::Event::KeyPressed>())
            {
                ...
                case sf::Keyboard::Scancode::Q:
                    quadTree_visible = !quadTree_visible;
                    break;
        ...
        if (mainWindow.hasFocus())
        {
            ...
            for (particle &i : particles)
            {
                ...
                // Placing pointers to particles in nodes they belong to
            }
            if (quadTree_visible){quadTree.drawTree();}
            for (particle &i : particles)
            {
            ...
            {
                std::clog << "Time:" << Timer.getElapsedTime().asMicroseconds()/1000000.0 << std::endl;
            }
        }
    }
    return 0;
