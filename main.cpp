    ...
    while (mainWindow.isOpen())
    {
        ...
        while (const std::optional event = mainWindow.pollEvent())
        {
            ...
            else if (const auto *key = event->getIf<sf::Event::TextEntered>())
            {
                ...
                {
                    std::clog << static_cast<unsigned short int>(key->unicode) << std::endl;
                }
                else if (key->unicode == 27)
                {
                    mainWindow.close();
                }
            }
        ...
        if (mainWindow.hasFocus())
        {
            ...
            else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Backspace) && particles.size() > 0)
            {
            ...
            {
                std::clog << "S:" << Timer.getElapsedTime().asMicroseconds()/1000000.0 << std::endl;
            }
        }
    }
    return 0;
